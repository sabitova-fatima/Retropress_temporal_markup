...
mon <- fromJSON(file = "dmonth.json") # the dictionary file
week <- fromJSON(file = "dweek.json") # the dictionary file
...
flags_num <- fromJSON(file = "dnum.json")
...
return_date_str <- function(input)
{
  ...
  split <- strsplit(res, '\n')
  split <- unlist(strsplit(as.character(split), ' '))
  split <- unlist(strsplit(as.character(split), ','))
  ...
  day <- 27;
  for (i in 1:length(split)){
    is_day <- ((str_detect(split[i], "\\d\\d") || str_detect(split[i], "\\d")) && i != year_index)
    if (is.na(is_day && (split[i+1] != "утра" && split[i+1] != "дня" && split[i+1] != "вечера" && split[i+1] != "часов" && split[i+1] != "минут"))){
      day = "11"
    } else if (is_day && (split[i+1] != "утра" && split[i+1] != "дня" && split[i+1] != "вечера" && split[i+1] != "часов" && split[i+1] != "минут")){
...
if(FALSE) {
  test <- "за сегодняшний день" # 2020-11-28 - 2020-11-30
  test <- "на протяжении сегодняшнего дня" 
  test <- "до августа тысяча четвертого года" # ???
  test <- "в этот уикенд"
  test <- "в субботу-воскресенье"
  test <- "всю эту неделю"
  test <- "неделю напролет"
  test <- "на этой неделе"
  test <- "на протяжении второго квартала две тысячи восемнадцатого года"
  ...
  now <- c("нынешний", "текущего", "текущий", "уходящего", "уходящий")
  
  week_flags <- fromJSON(file = "dweek.json")
  week_names = names(week)
  
  input = "в субботу-воскресенье"
  input <- unlist(strsplit(input, ' '))
  period_week <- function(input){
    for (i in 1:length(input)){
      for (j in 1:length(week_names)) {
        if (input[i] == week_names[j]) {
          week <- as.character(week[which(week_names == week_names[j])])
        }
      }
    }
    week = str_split_fixed(week, '-', 2)
    return(week)
  }
  
  period(input)
  
  this_week <- data.frame("0")
  this_week$dw1 = "2020-11-23"
  this_week$dw2 = "2020-11-24"
  this_week$dw3 = "2020-11-25"
  this_week$dw4 = "2020-11-26"
  this_week$dw5 = "2020-11-27"
  this_week$dw6 = "2020-11-28"
  this_week$dw7 = "2020-11-29"
